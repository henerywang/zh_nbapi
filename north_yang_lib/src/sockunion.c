...
int sockunion_socket(const union sockunion *su)
{
	...
	if (sock < 0) {
		...
		printf( "Can't make socket for %s : %d",
				      ...
				      unsigned short port, ifindex_t ifindex)
{
	...
	if (ret < 0) {
		if (errno != EINPROGRESS) {
			...
			printf("can't connect to %s fd %d : %d",
		   ...
		   union sockunion *su_addr)
{
	...
	if (ret < 0) {
		char buf[SU_ADDRSTRLEN];
		printf( "can't bind socket for %s : %d",
			 sockunion_log(su, buf, SU_ADDRSTRLEN),
			 (errno));
	}
...
int sockopt_reuseaddr(int sock)
{
	...
	if (ret < 0) {
		printf(
			"can't set sockopt SO_REUSEADDR to socket %d errno=%d: %d",
			sock, errno, (errno));
		return -1;
	}
	return 0;
...
int sockopt_minttl(int family, int sock, int minttl)
{
	...
	if (family == AF_INET) {
		...
		if (ret < 0)
			printf(
				"can't set sockopt IP_MINTTL to %d on socket %d: %d",
				minttl, sock, (errno));
		return ret;
	}
#endif /* IP_MINTTL */
	...
	if (family == AF_INET6) {
		...
		if (ret < 0)
			printf(
				"can't set sockopt IPV6_MINHOPCOUNT to %d on socket %d: %d",
				minttl, sock, (errno));
		return ret;
	}
#endif
...
union sockunion *sockunion_getsockname(int fd)
{
	...
	if (ret < 0) {
		printf(
			 "Can't get local address and port by getsockname: %d",
			 (errno));
		return NULL;
	}
...
union sockunion *sockunion_getpeername(int fd)
{
	...
	if (ret < 0) {
		printf( "Can't get remote address and port: %d",
			 (errno));
		return NULL;
	}
