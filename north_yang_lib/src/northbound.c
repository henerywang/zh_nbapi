			       ...
			       int callback_implemented, bool optional)
{
	...
	if (!optional && valid && !callback_implemented) {
		printf( "missing '%s' callback for '%s'",
			 nb_cb_operation_name(operation), nb_node->xpath);
		return 1;
	}
		    ...
		    bool preserve_source)
{
	...
	if (ret != 0)
		printf( "%s: lyd_merge() failed", __func__);
						 ...
						 const struct lyd_node *dnode)
{
	if (dnode->schema->nodetype & LYD_NODE_TERM)
		printf("nb_config_diff: %s: %s: %s", context, path,
			   lyd_get_value(dnode));
	else
		printf("nb_config_diff: %s: %s", context, path);
}
...
{
	if (!dnode) {
		printf("nb_config_diff: %s: NULL", context);
		return;
	}
			...
			struct lyd_node **new_dnode)
{
	...
	err = lyd_new_path(candidate->dnode, ly_native_ctx, xpath, value,
			   options, &dnode);
    printf("xpath =%s, value =%s, option =%d\n", xpath, value, options);
	if (err) {
		printf( "%s: lyd_new_path(%s) failed: %d",
			  __func__, xpath, err);
		return NB_ERR;
	} else if (dnode) {
				      ...
				      bool *error)
{
	...
	for (size_t i = 0; i < num_cfg_changes; i++) {
		...
		if (!nb_is_operation_allowed(nb_node, change->operation)) {
			printf(" Xpath %s points to key node", xpath);
			if (error)
				*error = true;
			break;
		}
			...
			struct lyd_node *dnode)
{
	...
	printf( "northbound notify: %s", xpath);
...
#define explicit_compile false
	...
	for (size_t i = 0; i < nmodules; i++) {
		printf( "northbound: loading %s.yang",
		       modules[i]->name);
